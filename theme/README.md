...
Dark mode uses a green accent drawn from the green stroke in the profile mark (`#4faf90`) for tags, navigation markers, links, focus outlines, and annotation link highlights. Light mode uses its original pink accent (`#a72f6a`) and pale pink selection. Handwritten notes and arrows keep their blue/green/purple colors in both appearances. The shared palette belongs to `geo.css`; `home.css` keeps decorative note ink separate from link highlights and hover colors. Both support system appearance without JavaScript and neutral print colors.
